...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='fpc-', dir=cache) as temporary:
        ...
        with (output / 'compiler.log').open('wb') as log:
            ...
            if args.tests:
                for probe in ('KEYTEST', 'XMSTEST', 'XFERTEST', 'MEMXFER', 'CACHETEST', 'UNDOTEST', 'STORTEST', 'CLIPTEST', 'TABTEST', 'SESSPROB', 'SAVETEST', 'RECVTEST', 'MEMTEST'):
    ...
    if args.tests:
        artifacts.extend(['KEYTEST.exe', 'XMSTEST.exe', 'XFERTEST.exe', 'MEMXFER.exe', 'CACHETEST.exe', 'UNDOTEST.exe', 'STORTEST.exe', 'CLIPTEST.exe', 'TABTEST.exe', 'SESSPROB.exe', 'SAVETEST.exe', 'RECVTEST.exe', 'MEMTEST.exe'])
